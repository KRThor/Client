			...
			try:
				...
				if recvDATA == "REBOOT": # 컴퓨터 재실행 → 프로그램 재실행으로 변경
					...
					if self.ModelBackup != self.nowModel:
						...
						if (CodeSetup == 'CUP' and ClientName == "CLIENT0") or (CodeSetup == 'CONE' and ClientName == "CLIENT1") or (LINE == 'SS11' and CodeSetup == 'CUP' and ClientName == "CLIENT1" and self.nowModel == 'MODEL8'):  
				...
				while True:
					...
					if Socket_main.resultSession == True:
						...
						if Socket_main.nowModel == "MODEL1" or Socket_main.nowModel == "MODEL2" or Socket_main.nowModel == "MODEL3" or Socket_main.nowModel == "MODEL4" or Socket_main.nowModel == "MODEL5" or Socket_main.nowModel == "MODEL6" or Socket_main.nowModel == "MODEL7" or Socket_main.nowModel == "MODEL8" or Socket_main.nowModel == "MODEL9" or Socket_main.nowModel == "MODEL10" or Socket_main.nowModel == "MODEL11" or Socket_main.nowModel == "MODEL12" or Socket_main.nowModel == "MODEL13" or Socket_main.nowModel == "MODEL14" or Socket_main.nowModel == "MODEL15":
							...
							if (CodeSetup == "CUP" and ClientName == "CLIENT0" and (Socket_main.nowModel in EFFI.Det_ModelSetup['CUP'])) or (CodeSetup == "CONE" and ClientName == "CLIENT1" and (Socket_main.nowModel in EFFI.Det_ModelSetup['CONE'])) or (LINE == 'SS11' and CodeSetup == "CUP" and ClientName == "CLIENT1" and Socket_main.nowModel == "MODEL8"):
	...
	def Det_inspection(self, image, boxlimit, frame):
		try:
			...
			for (box, score, label) in zip(boxes, scores, labels):
				...
				midX = startX + int((w)/2)
				midY = startY + int((h)/2)
				degree = math.degrees(math.atan2(-midY + 512, midX - 512))

				if x < 0: x = 0
				if y < 0: y = 0

				if y2 > classImage.shape[1] :
					y2 = classImage.shape[1]
				if x2 > classImage.shape[0]:
					x2 = classImage.shape[0]
		...
		try:
			...
			if FrameCount in [5,10,15]:
				...
				if (CodeSetup == "CUP" and ClientName == "CLIENT0" and (Socket_main.nowModel in EFFI.Det_ModelSetup['CUP'])) or (CodeSetup == "CONE" and ClientName == "CLIENT1" and (Socket_main.nowModel in EFFI.Det_ModelSetup['CONE'])) or (LINE == 'SS11' and CodeSetup == "CUP" and ClientName == "CLIENT1" and Socket_main.nowModel == "MODEL8"):
					...
					if EFFI.Det_Running == True:
						# SS11라인 101 식별 각인 추가
						if (LINE == 'SS11' and CodeSetup == "CUP" and ClientName == "CLIENT1" and Socket_main.nowModel == "MODEL8") :
							...
							if EFFI.EngraveCheckLimit_101[Socket_main.nowModel] == resultEngraveCount:
								...
								if self.Det_resultLabel[1] != EFFI.EngraveCheckLimit_101[Socket_main.nowModel]:
									self.Det_resultLabel[1] = resultEngraveCount
